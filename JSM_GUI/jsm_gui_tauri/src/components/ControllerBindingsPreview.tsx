...
type ControllerBindingsPreviewProps = {
  configText: string
  devices: TelemetryDevice[]
  ignoredDevices?: string[]
}

type ControllerBindingDeviceCardProps = {
  configText: string
...
function ControllerBindingDeviceCard({
  configText,
  device,
  ignoredDevices,
  ...
  }, [boundButtonCards, layerBoundCommandSet, selectedCommand])

  const normalizedSelectedCommand = selectedCommand?.toUpperCase() ?? null

  useEffect(() => {
    if (!normalizedSelectedCommand) {
      return
    }
...
export function ControllerBindingsPreview({
  configText,
  devices,
  ignoredDevices,
}: ControllerBindingsPreviewProps) {
  return (
    <>
      {devices.map(device => (
        <ControllerBindingDeviceCard
          key={device.handle}
