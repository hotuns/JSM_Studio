...
fn sync_child_state(process_state: &mut ProcessState) -> Result<(), String> {
  let exited = match process_state.child.as_mut() {
    Some(child) => match child.try_wait() {
      ...
      Ok(None) => false,
      Err(error) => {
        return Err(format!("Failed to inspect JoyShockMapper process state: {error}"));
      }
    },
    None => false,
  };
...
fn spawn_hidden_jsm_windows(
  ...
  let mut command_line = build_windows_command_line([executable.as_os_str(), runtime_dir.as_os_str()]);
  ...
  Ok(ManagedProcess::new(process_info.dwProcessId, process_info.hProcess))
}
