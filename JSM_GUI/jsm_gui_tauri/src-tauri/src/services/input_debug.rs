...
mod platform {
  ...
  pub fn start(app: AppHandle) -> Result<InputDebugHookStatus, String> {
    ...
    let thread_id = match ready_rx.recv() {
      ...
      Err(error) => {
        let _ = join.join();
        let message = format!("Input debug hook thread exited before it became ready: {error}");
        let mut runtime = runtime()?;
        runtime.running = false;
        runtime.thread_id = None;
        runtime.join = None;
  ...
  pub fn stop() -> Result<InputDebugHookStatus, String> {
    let join = {
      ...
      if let Some(thread_id) = thread_id {
        ...
        if posted == 0 {
          runtime.message = Some("Failed to request input debug hook shutdown.".to_string());
          runtime.running = false;
          runtime.thread_id = None;
          return Ok(status_from_runtime(&runtime));
        }
    ...
    if let Some(join) = join {
      if join.join().is_err() {
        let mut runtime = runtime()?;
        runtime.message = Some("Input debug hook thread panicked while stopping.".to_string());
        return Ok(status_from_runtime(&runtime));
      }
    }
  ...
  fn run_hook_thread(event_tx: Sender<InputDebugEvent>, ready_tx: Sender<Result<u32, String>>) {
    ...
    unsafe {
      let _ = PeekMessageW(&mut bootstrap_msg, null_mut(), WM_USER, WM_USER, PM_NOREMOVE);
    }

    let keyboard_hook = unsafe { SetWindowsHookExW(WH_KEYBOARD_LL, Some(keyboard_proc), null_mut(), 0) };
      ...
      if let Some(action) = keyboard_action(wparam as u32) {
        ...
        emit_input_event(InputDebugEvent {
          ...
          summary: format!(
            ...
            if hook.flags & LLKHF_EXTENDED != 0 { " extended" } else { "" },
            if hook.flags & LLKHF_ALTDOWN != 0 { " alt" } else { "" }
          ),
        });
      }
    }
  ...
  fn mouse_event(message: u32, hook: MSLLHOOKSTRUCT) -> Option<InputDebugEvent> {
    ...
    match message {
      WM_LBUTTONDOWN => Some(mouse_button_event("left", InputDebugAction::Down, injected, position)),
      WM_LBUTTONUP => Some(mouse_button_event("left", InputDebugAction::Up, injected, position)),
      WM_RBUTTONDOWN => Some(mouse_button_event("right", InputDebugAction::Down, injected, position)),
      WM_RBUTTONUP => Some(mouse_button_event("right", InputDebugAction::Up, injected, position)),
      WM_MBUTTONDOWN => Some(mouse_button_event("middle", InputDebugAction::Down, injected, position)),
      WM_MBUTTONUP => Some(mouse_button_event("middle", InputDebugAction::Up, injected, position)),
