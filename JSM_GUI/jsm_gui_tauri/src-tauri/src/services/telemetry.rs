...
const CONTROLLER_RESCAN_INTERVAL_MS: u64 = 2500;
...
pub fn emit_calibration_status(
  ...
  app
    .emit("calibration-status", payload)
...
fn handle_health(app: &AppHandle, state: &AppState) -> Result<(), String> {
...

  {
    ...
    let reconnect_due = telemetry_state
      .last_reconnect_attempt
      .map(|attempted_at| {
        attempted_at.elapsed() >= Duration::from_millis(CONTROLLER_RESCAN_INTERVAL_MS)
      })
      .unwrap_or(true);

    if (!has_devices || !has_fresh_telemetry) && reconnect_due {
      telemetry_state.last_reconnect_attempt = Some(Instant::now());
      should_reconnect = true;
    } else if has_devices && has_fresh_telemetry {
      telemetry_state.last_reconnect_attempt = None;
    }
  }
  ...
  if should_reconnect {
    ...
    if backend == "SDL" && jsm_process::is_running(state)? {
      let _ = jsm_process::inject_console_command(app, state, "RECONNECT_CONTROLLERS")?;
    }
  }
