...
GYRO_ON_ALL, GYRO_OFF_ALL: Enable or disable gyro on all connected devices simultaneously
; ' , . / \ [ ] + - `
...
```GYRO_ON_ALL``` and ```GYRO_OFF_ALL``` work just like ```GYRO_ON``` and ```GYRO_OFF```, but affect **all** connected gyro devices simultaneously instead of only the controller the binding is on. This is designed to be used with multiple devices, i.e. an external gyro box, where a single button can arm or disarm every gyro at once without having to assign the binding on each device separately.
...
For more control over how sensitivity scales with speed, you can replace the linear interpolation between MIN\_GYRO\_SENS and MAX\_GYRO\_SENS with a shaped **acceleration curve**. Set **ACCEL\_CURVE** to one of the following values:

* **LINEAR** (default) - Sensitivity interpolates linearly between MIN\_GYRO\_SENS at MIN\_GYRO\_THRESHOLD and MAX\_GYRO\_SENS at MAX\_GYRO\_THRESHOLD. This is identical to the classic behaviour.
* **NATURAL** - An exponential curve that rises gradually at slow speeds and more sharply at higher speeds, mimicking how natural acceleration feels. Tuned by **ACCEL\_NATURAL\_VHALF** (default 200.0 deg/s) — the gyro speed at which sensitivity reaches the halfway point between MIN\_GYRO\_SENS and MAX\_GYRO\_SENS.
* **POWER** - A power law curve that saturates smoothly toward MAX\_GYRO\_SENS. Tuned by **ACCEL\_POWER\_VREF** (default 0.01 deg/s), the reference speed at which the curve is approximately 63% of the way to MAX\_GYRO\_SENS, and **ACCEL\_POWER\_EXPONENT** (default 0.5), which shapes the steepness of the ramp. Values below 1.0 give a faster initial rise; values above 1.0 give a slower start and steeper finish.
* **QUADRATIC** - A simple squared ramp from MIN\_GYRO\_SENS to MAX\_GYRO\_SENS. The ramp completes at MAX\_GYRO\_THRESHOLD and requires no additional parameters. This gives slower, more controlled sensitivity growth at low speeds compared to LINEAR.
* **SIGMOID** - An S-shaped curve with a gentle start, steep middle, and soft top-end. Tuned by **ACCEL\_SIGMOID\_MID** (default 20.0 deg/s), the speed at the curve's inflection point, and **ACCEL\_SIGMOID\_WIDTH** (default 8.0 deg/s), which controls how steep the central transition is. A smaller width produces a sharper S-shape.
* **JUMP** - Holds near MIN\_GYRO\_SENS until a threshold, then quickly rises to MAX\_GYRO\_SENS, making sensitivity snap upward for fast movements. The jump threshold is set by MAX\_GYRO\_THRESHOLD, and **ACCEL\_JUMP\_TAU** (default 1.5 deg/s) controls the softness of the rise — smaller values produce a sharper jump.

**Finally**, there are a bunch more settings you can tweak if you so desire:

* **GYRO\_SPACE** (default LOCAL) - Simple gyro aiming solutions will map one of your controller's gyro axes to your camera/cursor's horizontal axis and one to the vertical axis. That's the behaviour you'll get with JoyShockMapper when GYRO\_SPACE is set to "LOCAL". This is simple to implement and leaves no room for misinterpretation, but aiming can feel off as you tilt your controller more and more. If you'd prefer a more advanced reading of the gyro combined with the accelerometer to more naturally handle different controller positions, PLAYER\_TURN is the way to go. Or, if you prefer to lean your controller side to side to turn your camera, try PLAYER\_LEAN. Finally, WORLD\_TURN and WORLD\_LEAN account for gravity more strictly than the PLAYER\_* options. There is also YAW\_PLUS\_ROLL, which blends the controller's roll rotation into the horizontal (yaw) contribution. This is essentially the same as LOCAL space but you have an extra axis you can control. This can improve fine control and extend your range of motion, but also can make aiming feel more unstable. The blend ratio is controlled by ROLL\_CONTRIBUTION.
* **ROLL\_CONTRIBUTION** (default 0.0) - When ```GYRO_SPACE``` is set to ```YAW_PLUS_ROLL```, this controls how much of the controller's roll rotation is added to horizontal mouse movement, expressed as a percentage of the yaw sensitivity. A value of 100 means roll contributes equally to yaw. Negative values invert the roll direction. Valid range is -100 to 100.
...
* **GYRO\_SMOOTHING\_DECAY** (default OFF) - When set to ON, replaces the default rolling-average gyro smoothing with an exponential moving average (EMA) whose effective smoothing window shrinks as gyro speed increases. At zero speed, the full ```GYRO_SMOOTH_TIME``` is applied. As speed rises toward ```GYRO_SMOOTH_THRESHOLD```, the effective smoothing time tapers to zero following a quadratic (squared) curve, so slow jitter receives maximum smoothing while fast movements receive progressively less until they pass through unsmoothed at the threshold. This gives heavier stabilisation at low speeds without adding latency to fast inputs.
* **ONE\_EURO\_FILTER** (default OFF) - When added to a config, applies a One Euro adaptive low-pass filter to the gyro input. Unlike the rolling-average smoother, the One Euro Filter continuously adapts its cutoff frequency based on how fast the gyro signal is changing: at rest, it applies strong smoothing to suppress sensor jitter; during fast movements, the cutoff rises quickly so inputs pass through with minimal latency. This can be a good alternative to GYRO\_SMOOTH\_THRESHOLD and GYRO\_SMOOTH\_TIME for players who want noise reduction that gets out of the way during flicks without a sudden threshold transition.
* **ONE\_EURO\_MIN\_CUTOFF** (default 6.0) - Controls how aggressively the One Euro Filter smooths the gyro signal when you are barely moving. Lower values mean more smoothing at rest, which further suppresses sensor jitter and micro-tremor when holding still. Raise it if the filter feels too sluggish to respond to slow, deliberate tracking movements. Only has an effect when ```ONE_EURO_FILTER``` is ON.
* **ONE\_EURO\_SPEED\_COEFF** (default 0.3) - Controls how quickly the One Euro Filter opens up and lets fast movement through unsmoothed. A higher value makes the filter back off sooner as your gyro speed rises, so flicks feel snappier. A lower value keeps more smoothing active even at moderate speeds, which can help if you still feel jitter during tracking. Only has an effect when ```ONE_EURO_FILTER``` is ON.
* **GYRO\_ANGLE\_SNAP** (default 0.0 degrees) - When set to a non-zero value, gyro input that is aimed nearly straight horizontally or vertically will be locked to that axis, eliminating the slight diagonal drift that occurs when trying to make a perfectly straight movement. The value (range 0–45) defines the angle in degrees on either side of the horizontal and vertical axes within which snapping activates. A value of 0 disables snapping entirely.
* **GYRO\_ANGLE\_SNAP\_EASE** (default OFF) - When set to ON, smoothly blends into the angle snap using a gradual transition rather than cutting the off-axis component abruptly. This can reduce the jarring feel of the snap at the cost of a slightly less precise lock-on.
* **DECEL\_BRAKE\_STRENGTH** (default 0.0) - Applies a deceleration brake that temporarily reduces gyro output when the controller decelerates sharply, such as when you stop a fast flick. This can reduce cursor overshoot at the end of a movement. The value ranges from 0 (no braking) to 1 (maximum braking). The brake engages based on how quickly the gyro speed is dropping, as determined by ```DECEL_BRAKE_THRESHOLD```.
* **DECEL\_BRAKE\_THRESHOLD** (default 25.0 deg/s) - Sets the deceleration rate (in degrees per second, measured over a short window) that begins to engage the deceleration brake. Lower values make the brake trigger more easily; higher values require a sharper stop before braking kicks in.
...
* **IGNORE\_GYRO\_DEVICES** - A space-separated list of VID:PID pairs (in hexadecimal) whose gyro data should be ignored by JoyShockMapper, even if the device is otherwise detected and connected. This is useful when using multiple devices, such as an external gyro box, alongside a controller that already has its own gyro: you can suppress the gyro on a per-device basis to avoid conflicting input. For example: ```IGNORE_GYRO_DEVICES = 0x054c:0x0ce6```. Setting it to an empty value clears the list.
...
* **TELEMETRY\_ENABLED** (default OFF) - When set to ON, JoyShockMapper streams real-time gyro and sensitivity data over a local UDP socket. The JSM GUI automatically forces this ON in order to power its live readouts, such as the sensitivity curve visualization and other real-time gyro feedback. If you are not using the GUI however, there is generally no reason to set this manually unless you are building a separate tool that consumes the telemetry feed.
* **TELEMETRY\_PORT** (default 8974) - The UDP port on localhost that telemetry data is broadcast to. Only takes effect when ```TELEMETRY_ENABLED``` is ON.
